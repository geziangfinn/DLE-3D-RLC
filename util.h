 ...
 *                     /   \       \
 *                   s      s       s
 * 3. 下面是本项目中和树的数据结构有关的操作代码，包括树的构建、树的功能以及树的读取三个部分
 */
 ...
 * @return 二叉树节点的指针
 */
TreeNode* convert_to_node(float x, float y, int layer, float C, int ID){
...
TreeNode* construct_buffer_node(TreeNode* template_node, float x, float y){
    TreeNode* node = (TreeNode*)malloc(sizeof(TreeNode));
    node->right_child = NULL; //默认值
    node->left_child = NULL; //默认值
    node->father = template_node; //默认值
    node->x = round(x);
    node->y  = round(y);
    node->layer = template_node->layer;
    node->C = 0;
    node->extra_WL = 0;
    node->delay = 0;
    node->ID = 0;
...
TreeNode* construct_extraWL_node(TreeNode* template_node, float x, float y){
    TreeNode* node = (TreeNode*)malloc(sizeof(TreeNode));
    node->right_child = NULL; //默认值
    node->left_child = NULL; //默认值
    node->father = template_node; //默认值
...
TreeNode* construct_TSV_node(TreeNode* template_node, float x, float y){
    TreeNode* node = (TreeNode*)malloc(sizeof(TreeNode));
    node->right_child = NULL; //默认值
    node->left_child = NULL; //默认值
    node->father = template_node; //默认值
