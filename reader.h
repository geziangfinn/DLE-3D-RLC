...
float** read(const char *filename){
    ...
    while(fgetc(file) != EOF){ //只要文件没有读到结尾
        if(fscanf(file, "num sink %d", &numpins)){ //就进行字符传匹配
            break; //匹配到之后就跳处循环，更换匹配关键字为Coordinate
        }
    }

    float **sink_set = initialize(numpins, 4); //初始化点集数组(包括内存分配和初始赋值)
    int pos = 0; //用于 sink_set 指针的移动，pos ++ 表示点的更换

    while(fgetc(file) != EOF){ //只要文件没有读到结尾
        if(fscanf(file, "\tCoordinate: %f %f %f %f", *(sink_set + pos), *(sink_set + pos)+1, *(sink_set + pos)+2, *(sink_set + pos)+3)){ //就进行字符串匹配
            pos ++; //匹配到字符串后，对点集进行赋值(横坐标x和纵坐标y)，然后将pos移动到下一个点的位置
        }
//        char coordX[30];
//        char coordY[30];
//        if(fscanf(file, "\fCoordinate: %s %s %lf", coordX, coordY, *(sink_set + pos)+2)){
//            double temp;
//            temp = atof(coordX);
//            printf("%lf", temp);
////            *(*(sink_set + pos)) = atof(coordX);
////            *(*(sink_set + pos)+1) = atof(coordY);
//            pos ++;
//        }
    }
